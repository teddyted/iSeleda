/*
 * $Id: JXCategory.java 3475 2009-08-28 08:30:47Z kleopatra $
 *
...
import org.jdesktop.swingx.plaf.CategoryAddon;
import org.jdesktop.swingx.plaf.CategoryUI;
import org.jdesktop.swingx.tips.DefaultCategoryModel;
 ...
 */
public class JXCategory extends JXPanel {

  /**
   * JXCategory pluggable UI key <i>swingx/CategoryUI</i>
   */
  public final static String uiClassID = "swingx/CategoryUI";

  // ensure at least the default ui is registered
  static {
    LookAndFeelAddons.contribute(new CategoryAddon());
  }
  ...
  /**
   * Constructs a new <code>JXCategory</code> with an empty
   * CategoryModel
   */
  public JXCategory() {
    this(new DefaultCategoryModel(new Category[0]));
  }

  /**
   * Constructs a new <code>JXCategory</code> showing tips from the given
   * CategoryModel.
   *
   * @param model
   */
  public JXCategory(CategoryModel model) {
    this.model = model;
    updateUI();
  }
...
public void updateUI() {
    setUI((CategoryUI)LookAndFeelAddons.getUI(this, CategoryUI.class));
  }
   ...
   *
   * @return the CategoryUI object that implements the CategoryUI L&F
   */
  @Override
  public CategoryUI getUI() {
    return (CategoryUI)ui;
  }
  ...
  public void setCurrentCategory(int currentCategory) {
    if (currentCategory < 0 || currentCategory >= getModel().getCategoryCount()) {
      throw new IllegalArgumentException(
      "Current tip must be within the bounds [0, " + getModel().getCategoryCount()
        + "]");
    }

    int oldCategory = this.currentCategory;
    this.currentCategory = currentCategory;
    firePropertyChange(CURRENT_TIP_CHANGED_KEY, oldCategory, currentCategory);
  }
  ...
  public void nextCategory() {
    int count = getModel().getCategoryCount();
    
    
    if (count == 0) { return; }

    int nextCategory = currentCategory + 1;
  ...
  public void previousCategory() {
    ...
    int previousCategory = currentCategory - 1;
    if (previousCategory < 0) {
      previousCategory = count - 1;
    }
    setCurrentCategory(previousCategory);
  }

  /**
   * Pops up a "Category" dialog.
   *
  ...
  /**
   * Calls
   * {@link CategoryUI#createDialog(Component, JXCategory.ShowOnStartupChoice)}.
   *
   ...
   * @param parentComponent
   * @param choice
   * @return a JDialog to show this Category pane
   */
   ...
   * {@link JXCategory#showDialog(Component, ShowOnStartupChoice)} to save the
